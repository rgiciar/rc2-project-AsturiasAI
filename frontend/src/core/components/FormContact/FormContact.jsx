      ...
      {isSubmitSuccessful ? (
        ...
        <form className="form-contact" onSubmit={handleSubmit(submit)}>
          <div className="email-container">
            <input className="form-email" placeholder="EMAIL" {...register('email')} id="email" type="email" />
            {errors.email && <p className="error">{errors.email.message}</p>}
          </div>

          <div className="name-container">
            <input className="form-name" placeholder="NOMBRE" {...register('name')} id="name" />
            {errors.name && <p className="error">{errors.name.message}</p>}
          </div>

          <div className="affair-container">
            <input className="form-affair"
            placeholder="ASUNTO" {...register('Asunto')} id="Asunto" />
            {errors.name && <p className="error">{errors.name.message}</p>}
          </div>


          <div className="message-container"> 
            <textarea className="form-message" placeholder="MENSAJE" {...register('message')} id="message" rows={10} />
